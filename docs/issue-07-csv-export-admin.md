...
Every admin table (event participants, members, and subscribers once
issue #6 is built) is view-only. There's no way to export a participant
...
no new backend/RLS work is needed, since the data is already loaded into
